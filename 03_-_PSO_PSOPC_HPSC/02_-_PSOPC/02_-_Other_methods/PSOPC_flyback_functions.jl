# Functions for solving optimization problem with the PSOPC flyback method.
#
...
Update the particle velocity, between the bounds
    ...
    particle = update_velocity_PCPSO(dim_X, particle, Swarm_i, ω, c1, c2, c3, Ri, Vmin, Vmax)
...
Parameters:
    ...
    c3            (Float): passive congregation in the actual iteration,
                           for the social velocity

    Ri            (Array): particle selected randomly from the swarm
                           -- array type -- Vector{Float64}
...
Returns:
    ...
    particle          (Struct):   struct particle (propieties)
"""
function update_velocity_PCPSO(dim_X      ::Int64,
                               particle   ::Particle,
                               Swarm_i    ::Swarm,
                               ω          ::Float64,
                               c1         ::Float64,
...
#   uniform random sequence U(0, 1)
    r1, r2, r3 = rand(Uniform(), dim_X), rand(Uniform(), dim_X), rand(Uniform(), dim_X);
...
#   updating the particles velocity (new velocity)
    particle.V = ω*V +                                    # inertia
                 ...
                 c3*r3.*(Ri - particle.X);                # passive congregation
...
"""
Function that applies the PSOPC flyback method. Reference [1]

fun_g_value, X_g_value, population, Swarm_prop = PSOPC_flyback(fun, c_fun, 
                                                               Xbounds, opt_arg,
                                                               Vbounds,
                                                               num_particles,
                                                               k_max, ω,
...
Parameters:
    ...
    c3                (Array):  array with the initial and final values of the
                                constant c3. The value of c3 changes linearly
                                with the iterations. Acceleration constant
                                for passive congregation coefficient.
                                For default c3 = [0.5, 0.5] --> constant
                                -----------------------------------------------
                                c3 = [c3_initial, c3_final]
                                -- array type -- Array{Float64, 1}
...
"""
function PSOPC_flyback(fun            ::Function,
                       c_fun          ::Function,
...
#   minimum and maximum parameters of method
    ...
    c3_initial = c3[LOWER];    c3_final = c3[UPPER];
...
#           computes the parameter value for iteration k
            ...
            c3_value = linear_variation(k, k_max, c3_initial, c3_final);

#           particle selected randomly from the swarm
            R_i_particle = rand(DiscreteUniform(1, num_particles));
            R_i = deepcopy(population[R_i_particle].X); # particle Ri

#           update the particle velocity
            particle = update_velocity_PCPSO(dim_X, particle, Swarm_prop,
                                             ω_value, c1_value, c2_value, c3_value,
                                             R_i, Vmin, Vmax);
